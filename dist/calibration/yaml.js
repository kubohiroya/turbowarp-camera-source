...
export function parseYaml(source) {
    ...
    if (lines.length === 0)
        throw new YamlError('The document is empty.', 0);
...
function splitLines(source) {
    ...
    source
        ...
        if (content.length === 0)
            return;
        if (!started && content.startsWith('%'))
            return;
        if (content === '---' || content.startsWith('--- ')) {
            if (started)
                throw new YamlError('Only one document is read from a calibration file.', number);
            started = true;
            const rest = content.slice(3).trim();
            if (rest.length > 0)
                lines.push({ number, indent: 0, text: rest });
            return;
        }
        if (content === '...') {
            started = true;
...
class BlockReader {
    ...
    readBlock(indent, depth) {
        if (depth > MAXIMUM_DEPTH)
            throw new YamlError('The document is nested too deeply.', this.peek()?.number ?? 0);
        const first = this.peek();
        if (!first)
            throw new YamlError('Expected a value.', 0);
        if (first.text === '-' || first.text.startsWith('- '))
            return this.readSequence(indent, depth);
        if (findMappingColon(first.text) >= 0)
            return this.readMapping(indent, depth);
        this.index += 1;
    ...
    readMapping(indent, depth) {
        ...
        while (!this.done()) {
            ...
            if (colon < 0)
                throw new YamlError('Expected a "key: value" pair.', line.number);
            ...
            const nestedSequence = next !== undefined && next.indent === indent && (next.text === '-' || next.text.startsWith('- '));
        ...
        if (text.startsWith('[') || text.startsWith('{')) {
            ...
            while (!flowClosed(joined)) {
                ...
                if (!next || next.indent <= indent)
                    throw new YamlError('A bracket is never closed.', number);
                joined += ` ${next.text}`;
                this.index += 1;
            }
...
function findMappingColon(text) {
    ...
    for (let index = 0; index < text.length; index += 1) {
        ...
        if (character === '"' || character === "'")
            quote = character;
        else if (character === ':' && (index === text.length - 1 || text[index + 1] === ' '))
            return index;
    }
    return -1;
}
...
class FlowReader {
    ...
    readValue(depth) {
        if (depth > MAXIMUM_DEPTH)
            throw new YamlError('The document is nested too deeply.', this.number);
        this.skipSpace();
        const character = this.text[this.index];
        if (character === '[')
            return this.readSequence(depth);
        if (character === '{')
            return this.readMapping(depth);
    ...
    readSequence(depth) {
        ...
        for (;;) {
            ...
            if (separator === ']')
                return sequence;
            if (separator !== ',')
                throw new YamlError('Expected "," or "]" in a sequence.', this.number);
            this.skipSpace();
    ...
    readMapping(depth) {
        ...
        for (;;) {
            ...
            this.skipSpace();
            if (this.text[this.index] !== ':')
                throw new YamlError('Expected ":" after a key.', this.number);
            this.index += 1;
            if (Object.prototype.hasOwnProperty.call(mapping, key)) {
            ...
            if (separator === '}')
                return mapping;
            if (separator !== ',')
                throw new YamlError('Expected "," or "}" in a mapping.', this.number);
            this.skipSpace();
            if (this.text[this.index] === '}') {
                this.index += 1;
                return mapping;
...
function readScalar(text, number) {
    ...
    const first = value[0];
    if (first === '"')
        return readDoubleQuoted(value, number);
    if (first === "'") {
        if (value.length < 2 || !value.endsWith("'"))
            throw new YamlError('A quoted string is never closed.', number);
        return value.slice(1, -1).replace(/''/g, "'");
    }
    if (first === '&' || first === '*')
        throw new YamlError('Anchors and aliases are not read.', number);
    if (first === '!')
        throw new YamlError('Tags are not read.', number);
    if (first === '|' || first === '>')
        throw new YamlError('Block scalars are not read.', number);
    if (first === '@' || first === '`')
        throw new YamlError(`A plain value cannot start with "${first}".`, number);
        ...
        switch (escape) {
            ...
            case 'u': {
                ...
                if (!/^[0-9a-fA-F]{4}$/.test(hex))
                    throw new YamlError('Invalid \\u escape.', number);
