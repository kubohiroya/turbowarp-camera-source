 ...
 */
export function serializeCameraInfoYaml(profile) {
    ...
    if (profile.device !== undefined) {
        lines.push('  device:');
        if (Object.keys(profile.device).length === 0)
            lines[lines.length - 1] += ' {}';
 ...
 */
export function readCameraInfoYaml(text) {
    let parsed;
    try {
    ...
    try {
        ...
        if (error instanceof CameraInfoRejection)
...
function toProfileDocument(parsed) {
    ...
    for (const key of Object.keys(root)) {
        if (!ROOT_KEYS.has(key))
            refuse('unexpected-field', key, 'Unknown member.');
    }
    readBinningAndRoi(root);
    const width = root['image_width'];
    const height = root['image_height'];
    if (width === undefined)
        refuse('missing-field', 'image_width', 'Required member is missing.');
    if (height === undefined)
        refuse('missing-field', 'image_height', 'Required member is missing.');
    ...
    if ('rectification_matrix' in root) {
        ...
        expectEntries(r, 'rectification_matrix', [1, 0, 0, 0, 1, 0, 0, 0, 1].map((value, index) => [index, value]), 'A rectification other than identity belongs to a stereo pair, not to one camera\'s intrinsics.');
    }
    if ('projection_matrix' in root)
        readMatrix(root, 'projection_matrix', 3, 4);
    const distortion = readDistortion(root);
    const extension = root[CAMERA_INFO_EXTENSION_KEY];
    if (extension === undefined) {
        refuse('missing-field', CAMERA_INFO_EXTENSION_KEY, 'The file is a ROS camera_info document without the calibration record: when it was calibrated and under which camera settings are not known.');
    }
    ...
    for (const key of Object.keys(extra)) {
        if (!known.has(key))
            refuse('unexpected-field', `${CAMERA_INFO_EXTENSION_KEY}.${key}`, 'Unknown member.');
    }
...
function rosDistortion(profile) {
    ...
    switch (profile.distortion.model) {
        ...
        case 'brown-conrady':
            if (coefficients.length === 8)
                return { model: RATIONAL_POLYNOMIAL, coefficients };
...
function readMatrix(root, name, rows, cols) {
    const value = root[name];
    if (value === undefined)
        refuse('missing-field', name, 'Required member is missing.');
    const matrix = record(value, name);
    const declaredRows = matrix['rows'];
    const declaredCols = matrix['cols'];
    const data = matrix['data'];
    if (declaredRows !== rows)
        refuse('invalid-value', `${name}.rows`, `Expected ${rows}.`);
    if (cols !== undefined && declaredCols !== cols)
        refuse('invalid-value', `${name}.cols`, `Expected ${cols}.`);
    ...
    if (!Array.isArray(data))
        refuse('invalid-type', `${name}.data`, 'Expected a sequence of numbers.');
...
//# sourceMappingURL=camera-info.js.map